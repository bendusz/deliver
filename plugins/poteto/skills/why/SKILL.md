...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, the IDE selection when one is attached, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the available MCP servers. In Claude Code the MCP tools are named `mcp__<server>__<tool>`; enumerate them with `ToolSearch` (deferred tools are listed by name in system reminders) and read each server's instructions block.
...
- `model`: `sonnet`
- **Do not use the `Explore` agent here.** MCP-backed investigators need the MCP tools, and each investigator must load its server's schemas via `ToolSearch` before calling them. Investigators still shouldn't write anything. That's a posture, not a sandbox.
