---
name: architect
description: "Sketch types, signatures, and module structure before code, then stay in the loop while implementation fills in. Use for /poteto:architect, 'architect this', 'design this', or non-trivial work where jumping to code would lock in the wrong shape."
...
Use `opus`, `fable`, and `sonnet` as the architect runners, optionally adding one Codex CLI runner for cross-family diversity.
...
Opt in to a checkpoint when the invoker explicitly asks: "/poteto:architect with checkpoint," "stop and show me before implementing," or similar. Then surface the synthesized design and pause for sign-off.
