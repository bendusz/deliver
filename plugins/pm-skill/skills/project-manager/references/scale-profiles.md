...
| --- | --- | --- |
| `standard` (default) | most projects | The baseline: `docs/spec.md`, `docs/plan.md`, `/pm-skill:analyze`, story files, a risk-selected review panel, and the project wiki maintained by `librarian`. |
| `tiny` | a one-off fix or tiny script | A minimal `docs/plan.md` and one story file. No spec, `/pm-skill:analyze`, checklists, verification reports, or wiki. One reviewer rather than a panel. |
| `small` | a small feature | A light spec, plan, and stories. No `/pm-skill:analyze` and no wiki. |
