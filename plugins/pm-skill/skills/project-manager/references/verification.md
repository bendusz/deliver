...
- If the project has a skill that drives the real app (for example one generated by
  `poteto:create-verification-skill` at `.claude/skills/verify-<app>/`), its path. The verifier
  launches the app through it, drives the story's feature the way a user would, and cites the
  captured evidence (screenshots, transcripts, response bodies) instead of reasoning from the diff
  alone.
