...
- If the project has a skill that drives the real app (for example one generated by
  `poteto:create-verification-skill` at `.claude/skills/verify-<app>/`), **you run it** for the
  story's feature before dispatching the verifier — the same way you run the gates — and pass the
  evidence paths it captured (screenshots, transcripts, response bodies). `pm-verifier` is
  read-only and never launches or drives the app itself; it checks that evidence against the
  acceptance criteria instead of reasoning from the diff alone.
