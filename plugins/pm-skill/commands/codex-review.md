...
message so they run in parallel, each with `Scope`, `Out dir`, `Objective` (a preset name or phrase),
and any `Model` or `Effort` overrides. The parsed `timeout=<minutes>` is in minutes and the agent's
`Timeout seconds` input is in seconds, so convert: pass
...
When two or more reports exist, write `<scope>-index.md` in the output directory, prefixed with the
same `YYYYMMDD-HHMMSS` stamp the runner used for the reports, listing each report path and its top
three findings. Then relay the digests.
