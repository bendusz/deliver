...
From the repository root (or the project directory for greenfield research), run once; retry at
most once only for a transient `failed` status:
...
transient. After reading the answer file, delete the scratch directory the envelope named (use
`rm -rf` on macOS and Linux, `Remove-Item -Recurse -Force` on Windows), using only the exact path
the envelope returned, so temp directories do not accumulate.
