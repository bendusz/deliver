...
This runs both builders and consumes both providers' usage. Never merge, push, or ship either
benchmark run.

Before starting:

- Require one tracked, build-ready story and a clean repository at a fixed base commit.
- Require valid story `pm-meta`, an approved and tracked `docs/approval.json`, an ignored `tmp/`, and passing
...
one for `codex-builder`, using names containing the timestamp so they cannot collide with delivery
...
most one clarification retry. Do not run the normal fix loop and do not let one worker see the other
worker's result.
...
only after several representative stories.
