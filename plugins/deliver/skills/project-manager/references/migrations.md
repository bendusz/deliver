...
**Enter** when a state read finds a pre-0.24 layout or an old story format. **Leave** when the
migration commit exists and the phase you were in resumes.
...
2. **Execution blocks.** For every story an `assignments` entry, an actor file's `current_story`,
   ...
   which an id from before 0.10.1 (the bare email local part) never matches by string; compare it
   with the slug prefix of `actor-id`'s output. Ids from 0.10.1 on are unchanged, the 0.21 rename
   included. A teammate's old id stays
...
4. **Handoffs.** Read each `pm/actors/*.HANDOFF.md`, `pm/HANDOFF.md`, or a pre-0.8 `tmp/HANDOFF.md`,
   once, and the old actor file's `next`. Fold anything still
...
5. **Remove.** `git rm -r pm/`, and the `tmp/` pointer stubs when present; delete the
   ...
   and add `Integration branch`, from the old `integration_branch` field, when it is missing. Now set
   `plan_digest` from
   `git hash-object docs/plan.md`, so the approved digest is the edited plan. The log stays in
   history: `git log -p -- pm/log.md`. A pre-0.8 `tmp/log.md` was never tracked, so fold anything
   still true from it into Execution notes first.
