...
`/deliver:doctor` flags. Without any identity the command exits non-zero, so set
...
- `signed_off` is load-bearing and global. While it is `false`, the `require-signoff.mjs` hook blocks
  ...
  `/deliver:correct-course` may set it back to `false` for a material change.
...
- Claims are visible, not locked, because git cannot make them atomic. A same-minute double-claim
  races. `/deliver:doctor` and `/deliver:analyze` report the race after one fetch, and a person
