...
each back. Run `/deliver:doctor` before the first sprint on a fresh or unfamiliar clone.
...
External review is optional; `/deliver:codex-review` owns it. **Log** a skip. With the gates green
...
`pm-verifier` `PASS`, and the outcome logged. `/deliver:correct-course` owns scope changes and the
