...
under `docs/` stay authoritative; the approval marker, Execution blocks, and handoffs are not wiki
content.
...
- Commit wiki changes together with the work they describe.
...
| After `/deliver:correct-course` | `librarian ingest` over the revised artifacts; the librarian supersedes |
...
## Readers
- On resume, read `docs/wiki/index.md` after git state and the handoff, before any `docs/` scan.
