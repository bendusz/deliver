...
- Once a sprint's stories are merged, you may dispatch `technical-writer` to refresh the user-facing
  ...
  story paths, and a history excerpt you write to `tmp/technical-writer/<sprint>.md` from
  `git log --first-parent` over the sprint. Never point it at the whole history or at earlier
  sprints' stories.
...
- Say in the sprint's closing commit that the writer ran, or that you skipped it. Never skip
  silently.
