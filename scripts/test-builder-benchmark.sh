...
SCORER="$ROOT/plugins/pm-skill/scripts/score-builder-benchmark.sh"
...
"$SCORER" "$tmp/results.json" > "$tmp/summary.md" || { echo "test-builder-benchmark: scorer rejected a valid fixture" >&2; exit 1; }
...
printf '{}\n' > "$tmp/invalid.json"
if "$SCORER" "$tmp/invalid.json" >/dev/null 2>&1; then echo "test-builder-benchmark: invalid result was accepted" >&2; exit 1; fi
